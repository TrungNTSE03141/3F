...
namespace _3F.Models.ContextModels
{
    public class RelationshipContext
    {
        ...
        /// <returns></returns>
        public List<RelationshipViewModels> GetListFriend(int userId)
        {
            //TODO
            //Need to optimation
            var result = from r in Relationships
        ...
        public CONST.FRIEND_OPTIONS CheckRelationship(int user_one_id, int user_two_id)
        {
            var result = from r in Relationships
                         where (r.user_one_id == user_one_id && r.user_two_id == user_two_id) 
                                || (r.user_one_id == user_two_id && r.user_two_id == user_one_id)
            ...
            if(result.Count() != 0)
            {
                if (result.ToList().ElementAt(0).status == 1)
                    return CONST.FRIEND_OPTIONS.Friends;
                if (result.ToList().ElementAt(0).action_user_id == user_one_id)
                    return CONST.FRIEND_OPTIONS.Request;
                return CONST.FRIEND_OPTIONS.Confirm;
            }
            return CONST.FRIEND_OPTIONS.None;
        }
