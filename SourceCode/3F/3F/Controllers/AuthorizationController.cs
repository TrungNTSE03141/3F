...
namespace _3F.Models
{
    public class AuthorizationController : Controller
    {
        ...
        private bool IsAccountValid(string email, string password)
        {
            ...
            if (user != null)
            {
                UserProfileView model = new UserProfileView(user.Id,user.avatarURL,user.address,user.username,user.dateOfBirth.ToString(),user.firstName+" "+user.middleName+" "+user.lastName);
